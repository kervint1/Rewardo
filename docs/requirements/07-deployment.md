...
```bash
# アプリ作成（リージョンはus。名前は例）
heroku create rewardo-api

# Postgresアドオン追加（Essential-0。1万行上限に注意、増えたらessential-1へ）
heroku addons:create heroku-postgresql:essential-0 -a rewardo-api
# → DATABASE_URL が自動で設定される

# 環境変数
heroku config:set -a rewardo-api \
...
# 確認
curl https://rewardo-api-<hash>.herokuapp.com/health
```
...
| `NEXT_PUBLIC_API_URL` | HerokuアプリのURL（例: `https://rewardo-api-xxx.herokuapp.com`） |
...
- [ ] 管理者運用: TablePlus等で `heroku config:get DATABASE_URL -a rewardo-api` の接続文字列を使ってHeroku Postgresに接続

## 4. Monlix接続時（契約後）

- [ ] Postback URL: `https://rewardo-api-xxx.herokuapp.com/postback/monlix?userid={subid}&transaction_id={tid}&amount={amount}&status={status}&hash={hash}`（マクロ名はMonlix仕様に合わせる）
