...
| 残高・換金履歴の表示 | フロントエンド → FastAPI → Heroku Postgres | 自前JWTで本人確認し、自分のデータだけ返す |
| 報酬付与（Postback） | Monlix → FastAPI → Heroku Postgres | 二重付与防止・署名検証はサーバー側で実施 |
| 換金申請 | フロントエンド → FastAPI → Heroku Postgres | 残高チェックと差し引きをアトミックに行う |
...
| GET | `/api/v1/me` | 自前JWT | 自分のプロフィール・残高取得 |
...
### GET `/api/v1/me` — 自分の情報と残高

Home画面のヘッダー（所持金・プログレスバー）とWallet画面の残高表示に使う。
...
`min_withdrawal_amount` を含めるのは、フロントエンドがプログレスバーと申請ボタンの活性制御に使うため（値をフロントにハードコードしない）。
...
| `amount >= 10.00`（最低換金額 S/ 10.00） | 422 `BELOW_MINIMUM` |
| `amount <= users.balance` | 422 `INSUFFICIENT_BALANCE` |
| 同一ユーザーの `pending` 申請が存在しない | 409 `WITHDRAWAL_ALREADY_PENDING` |

**処理**（1トランザクションで実行。残高は**申請時に差し引く**）

```
BEGIN;
SELECT balance FROM users WHERE id = :user_id FOR UPDATE;  -- 行ロックで並行申請を防ぐ
-- 残高チェック
UPDATE users SET balance = balance - :amount WHERE id = :user_id;
INSERT INTO withdrawals (user_id, yape_phone, amount, status) VALUES (..., 'pending');
COMMIT;
```
...
```
BEGIN;
INSERT INTO postbacks (transaction_id, user_id, reward_amount) VALUES (...);
...
```
申請時:    balance から amount を差し引き、withdrawals に pending で保存
completed: 管理者がYape送金後、DBクライアントで status を completed に変更（残高操作なし）
rejected:  管理者がDBクライアントで status を rejected に変更し、
           あわせて balance に amount を戻すSQLを実行（返金）
```

rejected時の返金SQL（DBクライアントから実行）:
...
> ⚠️ MVPでは手動運用。件数が増えたらDBトリガー（`status` が `rejected` に変わったら自動返金）に置き換える。

残高を申請時に差し引く理由: 差し引かないと、`pending` 中に残高を使い回して二重申請できてしまう。「pending申請は1件まで」の制約と合わせて二重出金を防ぐ。
...
sequenceDiagram
    ...
    API->>API: 署名検証
    API->>DB: INSERT postbacks + UPDATE users.balance（1トランザクション）
    API-->>M: 200 OK
    U->>FE: 画面更新
...
sequenceDiagram
    ...
    U->>FE: /wallet でYape番号と金額を入力
    FE->>API: POST /api/v1/withdrawals（JWT付き）
    API->>API: JWT検証・バリデーション
    API->>DB: balance差し引き + withdrawals作成（1トランザクション）
    API-->>FE: 201 pending
...
| `MIN_WITHDRAWAL_AMOUNT` | 最低換金額。デフォルト `10.00` |
