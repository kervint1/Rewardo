...
| `points >= 10000`（最低換金ポイント = S/ 10相当） | 422 `BELOW_MINIMUM` |
| `points` が `POINTS_PER_SOL`（1,000）の倍数（端数ソルを発生させない） | 422 `INVALID_AMOUNT` |
...
| `POINTS_PER_SOL` | 換金レート。デフォルト `1000`（1,000 pts = S/ 1） |
| `MIN_WITHDRAWAL_POINTS` | 最低換金ポイント。デフォルト `10000`（= S/ 10） |
